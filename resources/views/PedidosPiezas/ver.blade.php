...
<style>
  ...
  /* Color para 'No aplica' */
  .text-na {
  color: #888 !important;    /* Gris */
  font-style: italic !important; /* Asegura que no sea itálica */
}
    ...
    <div class="col-12 col-xl-10">
      ...
      <header class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 p-3 bg-white rounded shadow-sm">
        ...
        <div class="d-flex align-items-center mb-3 mb-md-0 me-md-4">
          ...
          <div>
            <h1 class="h4 fw-bold mb-0">Gestión de Pedidos de Piezas</h1>
            <p class="text-muted small mb-0">Administración de repuestos entre sucursales</p>
          </div>
        </div>
        ...
        <div class="d-flex flex-column flex-sm-row align-items-stretch align-items-sm-center gap-3 w-100 w-md-auto" style="max-width: 500px;">
          ...
          <form action="#" method="GET" class="flex-grow-1 position-relative">
            <div class="input-group input-group-sm">
              ...
              <input type="text" name="q" value="#" 
                     ...
                     placeholder="Buscar pedido o pieza">
          ...
          <!-- Botón NUEVO PEDIDO -->
          <a href="{{ route('pedidos_piezas.crear') }}"
      ...
      @if(isset($busqueda) && $busqueda)
        <div class="alert alert-info alert-dismissible fade show mb-3" role="alert">
          ...
          <a href="{{ route('pedidos_piezas.ver') }}" class="btn btn-sm btn-link">Limpiar</a>
...
@endif
      ...
      <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
            ...
            <table class="table table-hover align-middle mb-0">
              <thead class="bg-light">
                <tr>
                  ...
                  <th class="fw-semibold text-center">Refacciones</th>
                  <th class="fw-semibold text-center">Fecha Envío</th>
                  <th class="fw-semibold text-center">Acciones</th>
                </tr>
              </thead>
              <tbody>
                @forelse ($pedidosPiezas->groupBy('id_pedido') as $id_pedido => $items)
                  @php
                  ...
                  <tr class="pedido-row">
                    <td class="text-muted fw-semibold">#{{ $id_pedido }}</td>
                    <td>
                      <span class="d-block fw-semibold">{{ $pedido->sucursal->nombre_sucursal ?? 'N/D' }}</span>
                    </td>
                    ...
                    <td class="text-center">
                      <div class="d-flex justify-content-center gap-2">
                        <button class="btn btn-outline-secondary btn-sm rounded-pill px-3" 
                                ...
                                data-bs-target="#detalle-{{ $id_pedido }}">
                          <i class="bi bi-eye me-1"></i>Ver
                        </button>
                        
                        <a href="{{ route('pedidos_piezas.edit', $items->first()->id_control) }}"
                          class="btn btn-outline-primary btn-sm rounded-pill px-3">
                          <i class="bi bi-pencil-square me-1"></i>Editar
                        </a>

                       <a href="{{ route('pedidos_piezas.pdf', $pedido->id_pedido) }}" 
                            target="_blank"
                  ...
                  <!-- DETALLE PIEZAS -->
                  <tr class="collapse collapse-row detalle-expandido" id="detalle-{{ $id_pedido }}">
                    <td colspan="6" class="p-3">
                      <div class="d-flex align-items-center mb-3">
                        <i class="bi bi-gear text-muted me-2"></i>
                        <h6 class="mb-0 text-muted">Detalle del pedido <strong>#{{ $id_pedido }}</strong></h6>
                      </div>
                          ...
                          <tbody>
                            @foreach ($items as $i => $detalle)
                              <tr>
                                ...
                                <td>
                                  <strong>{{ $detalle->pieza->nombre_pieza ?? 'N/D' }}</strong>
                                </td>

                                <td  class="fw-semibold">{{ $detalle->pieza->descripcion_general ?? 'Error' }}
                                </td>

                                <td class="{{ ($detalle->pieza->modelo->nombre_modelo ?? 'Error') === 'Sin Modelo' ? 'text-na' : 'text-dark fw-semibold' }}">
                                    {{ $detalle->pieza->modelo->nombre_modelo ?? 'Error' }}
                                </td>


                               <td class="{{ ($detalle->pieza->color ?? '') === 'No aplica' ? 'text-na' : 'text-dark fw-semibold' }}">
                                                    {{ $detalle->pieza->color ?? 'N/D' }}
                                                    </td>

                                <td  class="fw-semibold">{{ $detalle->pieza->Unidad ?? 'No hay' }}</td>
                  ...
                  <tr>
                    <td colspan="5" class="text-center py-5">
                      <div class="d-flex flex-column align-items-center text-muted">
                        ...
                        <span>No hay pedidos de piezas registrados</span>
                        <a href="{{ route('pedidos_piezas.crear') }}" class="btn btn-link mt-2">
                          Crear primer pedido
                        </a>
                      </div>
                    </td>
        ...
        @if($pedidosPiezas->hasPages())
          <div class="card-footer bg-white py-3">
            <div class="d-flex justify-content-center">
              {{ $pedidosPiezas->links('pagination::bootstrap-4') }}
            </div>
          </div>
        @endif
      </div>
