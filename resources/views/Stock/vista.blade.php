        ...
        <div class="col-lg-10 col-xl-8">
            ...
            @endif
             ...
             @if (auth()->user()->rol == 0)
                            <div class="text-center mb-3">
                                <a href="{{ route('Stock.crear') }}" class="btn btn-outline-success">
                        ...
                        <table class="table table-hover mb-0" id="tablaTipoStock">
                            ...
                            <tbody>
                                @forelse ($tipos as $tipo)
                                    <tr>
                                        <td class="text-center fw-semibold">{{ $tipo->id_tipoStock }}</td>
                                        <td class="text-center fw-semibold">{{ $tipo->nombre_stock }}</td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-outline-primary btn-sm" 
                                                data-bs-toggle="modal"
                                                data-bs-target="#modalActualizar{{ $tipo->id_tipoStock }}">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                <div class="text-muted small">
                    Mostrando {{ $tipos->firstItem() }} a {{ $tipos->lastItem() }} de {{ $tipos->total() }} registros
                </div>
                <nav aria-label="Page navigation">
                    <ul class="pagination pagination-sm justify-content-center">
                        {{-- Previous Page Link --}}
                        @if($tipos->onFirstPage())
                            <li class="page-item disabled">
                                <span class="page-link">&laquo;</span>
                            </li>
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ $tipos->previousPageUrl() }}" rel="prev">&laquo;</a>
                            </li>
                        @endif

                        {{-- Pagination Elements --}}
                        @foreach($tipos->getUrlRange(1, $tipos->lastPage()) as $page => $url)
                            @if($page == $tipos->currentPage())
                        ...
                        {{-- Next Page Link --}}
                        @if($tipos->hasMorePages())
                            <li class="page-item">
                                <a class="page-link" href="{{ $tipos->nextPageUrl() }}" rel="next">&raquo;</a>
                            </li>
                        @else
                            <li class="page-item disabled">
