...
<form method="POST" action="{{ route('Bicicleta.biciensistema') }}" class="row g-3 was-validated" id="formBicicleta">
    @csrf

    <div class="col-md-6">
        <label for="num_chasis_parcial" class="form-label">@lang('Últimos 10 Dígitos Alfanuméricos de Número de Chasis')</label>
        <input 
  type="text" 
  name="num_chasis_parcial" 
  id="num_chasis_parcial" 
  maxlength="10" 
  minlength="10" 
  pattern="[A-Za-z0-9]{10}" 
  title="Debe ingresar los últimos 10 caracteres del número de chasis" 
  class="form-control" 
  required 
  autocomplete="off"
>
        ...
        <div class="form-text">@lang('Al ingresar los últimos 10 caracteres, se buscará automáticamente la bicicleta.')</div>
    </div>
...
<a href="{{ url('/imprimir-bicis') }}" class="btn btn-primary" target="_blank">
    Imprimir todo
</a>
...
$(document).ready(function () {
    ...
    function mostrarAlertaModal(tipo, mensaje) {
        ...
        const iconPath = tipo === 'success'
            ? 'M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM6.97 11.03a.75.75 0 0 0 1.08-.02l3.99-4.99a.75.75 0 1 0-1.08-1.04L7.5 9.585 5.97 8.06a.75.75 0 0 0-1.08 1.04l2.08 1.93z'
            : 'M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm0 11a.5.5 0 0 1-.5-.5V8.707L5.354 6.561a.5.5 0 1 1 .707-.707L8 7.293l1.939-1.939a.5.5 0 1 1 .707.707L8.5 8.707V11.5a.5.5 0 0 1-.5.5z';
    ...
    $('#num_chasis_parcial').on('input', function () {
        // Convertir a mayúsculas y limitar a 10 caracteres alfanuméricos
        let val = this.value.toUpperCase();
        val = val.replace(/[^A-Z0-9]/g, ''); // Solo letras mayúsculas y números
        if (val.length > 10) {
            val = val.slice(0, 10);
        }
        this.value = val;
